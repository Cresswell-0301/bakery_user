...
const Orders = async () => {
    ...
    orders = orders?.sort((a: OrderType, b: OrderType) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
        ...
        <div className="px-10 py-5 max-sm:px-3">
            ...
            <div className="flex flex-col gap-10">
                {orders?.map((order: OrderType) => (
                    <div key={order.orderId} className="flex flex-col gap-8 p-4 hover:bg-grey-1">
                        <div className="flex gap-20 max-md:flex-col max-md:gap-3 justify-between">
                            ...
                            <p className="text-base-bold">{format(new Date(order.createdAt), "dd/MM/yyyy hh:mm a")}</p>
                        </div>

                        <div className="flex flex-col gap-5">
                            {order.products.map((orderItem: OrderItemType) => (
                                <div key={orderItem._id} className="flex gap-4">
                                    <ProductImage src={orderItem.product.media[0]} alt={orderItem.product.title} productId={orderItem.product._id} />
                                    <div className="flex flex-col w-full">
                                        ...
                                        <p className="text-small-medium flex flex-row justify-between text-gray-400">
                                            <span className="text-[14px] font-[500] leading-[140%]">{orderItem.product.sizes}</span>
                                        </p>
                                        <p className="text-small-medium flex flex-row justify-between">
                                            <span className="text-[14px] font-[500] leading-[140%]">
                                                {currencyCode[0].code} {orderItem.product.price.toFixed(2)}
                                            </span>

                                            <span className="font-bold text-gray-500 text-[12px]">x {orderItem.quantity}</span>
                                        </p>
                        ...
                        ${
                            orderItem.status === "paid" || orderItem.status === "refund"
                                ? "bg-green-500"
                                ...
                                : "bg-red-500"
                                            ...
                                            >
                                                {orderItem.status === "received_order" ? "Order Received" : orderItem.status.charAt(0).toUpperCase() + orderItem.status.slice(1)}
                                            </span>

                                            <span className="text-[14px] font-[500] leading-[140%] ml-auto">
                                                {currencyCode[0].code} {(orderItem.product.price * orderItem.quantity).toFixed(2)}
                                            </span>
                                        </p>
                                    </div>
                                </div>
                            ...
                            <p className="text-base-bold text-right">
                                Total {order.products.reduce((acc: number, item: OrderItemType) => acc + item.quantity, 0)} Items: {currencyCode[0].code}{" "}
                                {order.totalAmount.toFixed(2)}
                            </p>
                            {order.status === "paid" && (
                                <p className="flex justify-end text-base-bold text-right">
                                    + {order.totalAmount % 1 >= 0.99 ? Math.floor(order.totalAmount) + 1 : Math.floor(order.totalAmount)}
